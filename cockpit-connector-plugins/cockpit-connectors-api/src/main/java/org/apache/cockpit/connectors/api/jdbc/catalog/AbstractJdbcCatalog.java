...
import static org.apache.cockpit.connectors.api.catalog.exception.CommonErrorCode.UNSUPPORTED_METHOD;
...
//        }
        try {
            ...
            driverConfig.setJdbcDriverClass(driverClass);
            driverConfig.setJdbcDriver(driverLocation);
            Connection connection = IDriverManager.getConnection(url, username, pwd, driverConfig);
            connectionMap.put(url, connection);
            return connection;
    ...
    protected void createTableInternal(TablePath tablePath, CatalogTable table, boolean createIndex)
        ...
        try {
            ...
            if (catalogName.equalsIgnoreCase(DbType.CACHE.getCode())) {
                for (String sql : createTableSqlList) {
                    // 先分割 SQL 语句
                    String[] sqlArray = sql.split(";");
                    for (String singleSql : sqlArray) {
                        if (singleSql != null && singleSql.trim().length() > 0) {
                            executeInternal(dbUrl, singleSql.trim());
                        }
                    }
                }
            } else {
