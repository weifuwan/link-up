...
public class Hive3SinkFactory implements TableSinkFactory {
    ...
    public TableSink createSink(TableSinkFactoryContext context) {
        ...
        if (StringUtils.isNoneBlank(config.get(FileBaseSinkOptions.PARTITION_FIELD))) {
            String partitionField = config.get(FileBaseSinkOptions.PARTITION_FIELD);
        ...
        String fileFormatType = config.get(FileBaseSinkOptions.FILE_FORMAT_TYPE).toString();
        catalogTable
                .getOptions()
                .put("fieldIde", fieldIdeEnum == null ? null : fieldIdeEnum.getValue());
        catalogTable
                .getOptions()
                .put("file_format_type", fileFormatType);
    ...
    public OptionRule optionRule() {
        return OptionRule.builder()
                .required(THRIFT_URL, DRIVER, SCHEMA_SAVE_MODE, DATA_SAVE_MODE)
                .optional(
                        CREATE_INDEX,
                        USERNAME,
                        PASSWORD,
