...
export const dataSourceCatalogApi = {
    ...
    listTable: (id: string): Promise<{ code: number; data: any; message?: string }> => {
        return HttpUtils.get(`${apiPrefixCatalog}/${id}`);
    },

    listColumn: (id: string, tableName: string): Promise<{ code: number; data: any[]; message?: string }> => {
        return HttpUtils.get(`${apiPrefixCatalog}/column/${id}?tableName=` + tableName);
    },
