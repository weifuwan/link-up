        ...
        {usedDataSource.map((ds, index) => (
          <Col span={7} key={index} style={{ padding: '0px 3px' }}>
            <div className="lf-ds-card" onClick={() => selectSource(ds?.value, true)}>
              <div className="lf-ds-logo">{<DatabaseIcons dbType={ds.label} />}</div>
              <div>{ds?.label}</div>
            </div>
          </Col>
        ))}
            ...
            group.datasourceList.map((ds) => {
              return (
                <Col span={8} key={index} style={{ padding: '0px 4px' }}>
                  <div className="lf-ds-card" onClick={() => selectSource(ds?.text, true)}>
                    <div className="lf-ds-logo">{<DatabaseIcons dbType={ds.text} />}</div>
                    <div>{ds?.text}</div>
                  </div>
                </Col>
              );
