...
import org.apache.cockpit.plugin.datasource.postgresql.param.PostgreSQLDataSourceProcessor;
...
public class PostgreSQLMetadataService {
    ...
    public static List<String> listTables(ConnectionParam connectionParam,
        ...
        try (Connection connection = processor.getConnection(connectionParam);
             PreparedStatement ps = connection.prepareStatement(getListTableSql(postgreSQLConnectionParam.getDatabase(),
                     postgreSQLConnectionParam.getSchema()));
    ...
    public static List<DataSourceTableColumn> listColumns(ConnectionParam connectionParam, String tableName, DataSourceProcessor processor) {
        ...
        PostgreSQLConnectionParam postgreSqlConnectionParam = (PostgreSQLConnectionParam) connectionParam;
        String schema = "public"; // PostgreSQL 默认 schema

        try (Connection connection = processor.getConnection(connectionParam);
             ResultSet rs = connection.getMetaData().getColumns(postgreSqlConnectionParam.getDatabase(), schema, tableName, null)) {
    ...
    public static QueryResult getTop10Data(ConnectionParam connectionParam, Map<String, Object> requestBody,
        ...
        try {
            ...
            switch (taskExecuteType) {
                case SINGLE_TABLE:
                    PostgreSQLConnectionParam pgConnectionParam = (PostgreSQLConnectionParam) connectionParam;
