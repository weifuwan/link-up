...
@Slf4j
public class OpenGaussQueryBuilder {
    ...
    public JSONObject buildSourceJson(String connectionParam, String sourceParam) {
        ...
        OpenGaussConnectionParam connectionParams = JSONUtils.parseObject(connectionParam, OpenGaussConnectionParam.class);
        ...
        if (StringUtils.isNotBlank(sourceParamObj.getString("table_path"))) {
            ...
            if (StringUtils.isNotBlank(tablePath)) {
                ...
                if (split.length == 1) {
                    tablePath = connectionParams.getDatabase() + "." + connectionParams.getSchema() + "." + split[0];
                }
            } else {
        ...
        jsonObject.put("plugin-type", DbType.POSTGRESQL.getCode());
        jsonObject.put("dialect", DbType.POSTGRESQL.getCode());
    ...
    public JSONObject buildSinkJson(String connectionParam, String globalParam) {
        ...
        OpenGaussConnectionParam connectionParams = (OpenGaussConnectionParam) JSONUtils.parseObject(connectionParam, OpenGaussConnectionParam.class);

        String sinkTablePath;
        if (sinkObj.getBoolean("autoCreateTable")) {
            String sourceTablePath = sourceObj.getString("table_path");
            String[] split = sourceTablePath.split("\\.");
            if (split.length == 2) {
                sinkTablePath = connectionParams.getUsername() + "." + split[1];
            } else if (split.length == 1) {
                sinkTablePath = connectionParams.getUsername() + "." + sourceTablePath;
            } else {
                sinkTablePath = sourceTablePath;
            }
        } else {
            sinkTablePath = sinkObj.getString("table_path");
            String[] split = sinkTablePath.split("\\.");
            if (split.length == 1) {
                sinkTablePath = connectionParams.getUsername() + "." + sinkTablePath;
            }
        }
        ...
        jsonObject.put("plugin-type", DbType.OPENGAUSS.getCode());
        jsonObject.put("dialect", DbType.OPENGAUSS.getCode());
        jsonObject.put("database", connectionParams.getDatabase());
        jsonObject.put("driver_location", connectionParams.getDriverLocation());
