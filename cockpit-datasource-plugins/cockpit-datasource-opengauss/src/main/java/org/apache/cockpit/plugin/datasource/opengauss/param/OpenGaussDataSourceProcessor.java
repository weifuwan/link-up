...
import org.apache.cockpit.plugin.datasource.opengauss.catalog.*;
...
@Slf4j
public class OpenGaussDataSourceProcessor extends AbstractDataSourceProcessor {

    private final OpenGaussConnectionManager connectionManager;
    private final OpenGaussQueryBuilder queryBuilder;

    public OpenGaussDataSourceProcessor() {
        this.connectionManager = new OpenGaussConnectionManager();
        this.queryBuilder = new OpenGaussQueryBuilder();
    }

    @Override
    public BaseDataSourceParamDTO castDatasourceParamDTO(String paramJson) {
        return JSONUtils.parseObject(paramJson, OpenGaussDataSourceParamDTO.class);
    }

    @Override
    public BaseDataSourceParamDTO createDatasourceParamDTO(String connectionJson) {
        return OpenGaussParamConverter.createParamDTOFromConnection(connectionJson);
    }

    @Override
    public BaseConnectionParam createConnectionParams(BaseDataSourceParamDTO dataSourceParam) {
        return OpenGaussParamConverter.createConnectionParamFromDTO(dataSourceParam);
    }

    @Override
    public ConnectionParam createConnectionParams(String connectionJson) {
        return JSONUtils.parseObject(connectionJson, OpenGaussConnectionParam.class);
    }
    ...
    public String getValidationQuery() {
        return DataSourceConstants.OPENGAUSS_VALIDATION_QUERY;
    }
    ...
    public DataSourceProcessor create() {
        return new OpenGaussDataSourceProcessor();
    }

    @Override
    public List<String> splitAndRemoveComment(String sql) {
        return OpenGaussQueryParser.splitAndRemoveComment(sql);
    }
    ...
    public List<String> listTables(ConnectionParam connectionParam) {
        return OpenGaussMetadataService.listTables(connectionParam, this);
    }

    @Override
    public List<DataSourceTableColumn> listColumns(ConnectionParam connectionParam, String tableName) {
        return OpenGaussMetadataService.listColumns(connectionParam, tableName, this);
    }

    @Override
    public QueryResult getTop20Data(ConnectionParam connectionParam, Map<String, Object> requestBody) {
        return OpenGaussMetadataService.getTop10Data(connectionParam, requestBody, this);
    }
}